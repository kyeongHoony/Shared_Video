...
class MotionGuidedFrameSelector:
    ...
    def select_frames_by_threshold(
        ...
        total_frames: int,  # noqa: ARG002 — kept for API compatibility with original
...
class SpatialPatchAnalyzer:
    ...
    def analyze_frame_patches(self, flow: np.ndarray, frame_idx: int) -> Dict:  # noqa: ARG002
        h, w = flow.shape[:2]
...
class JetsonSpatioTemporalOptimizer:
    ...
    def load_model(self):
        ...
        self.model = AutoModelForVision2Seq.from_pretrained(
            self.model_path,
            config=config,
            torch_dtype=torch.bfloat16,     # bfloat16: Orin Ampere supports natively, standard for modern LLMs
            ...
            attn_implementation="sdpa",     # uses F.scaled_dot_product_attention → flash_sdp backend on Jetson
        )
        self.model.eval()
    ...
    def optimize_video(self, video_name: str, query: str = "Describe this video.") -> OptimizationMetrics:
        ...
        # Stage 3: Spatial analysis + mask generation
        ...
        if self.model is not None:
            ...
            # Stage 4: Mask application + PIL conversion (revised: non-important patches zeroed)
            t0 = time.time()
            masked_frames = [
            ...
            stage_times["4_mask_apply_pil_conversion"] = time.time() - t0
